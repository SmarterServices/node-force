...
module.exports = schema;`;
  ...
  getJoiSchema() {
    ...
    var joiProperties = [];
    const unncessaryFields = ['Id', 'sid', 'id', 'isDeleted', 'createdDate', 'lastModifiedDate', 'systemModstamp'];
    ...
    this.syncedForceFields.forEach(function forEachForceField(forceField) {
      ...
      if (unncessaryFields.includes(fieldName)) {
        return;
      }
...
const customSchema = require('./custom-schema/${this.fileName}');
const validation = require('./validation/${this.fileName}');
const customValidation = require('./custom-validation/${this.fileName}');

module.exports = function (sequelize, schemaName) {
  return sequelize.define('${_.toLower(this.modelName)}',
