...
var utils = {
  ...
  /**
   * Generate endpoint from configuration and writes to the files
   * @returns {Promise}
   */
  generateEndpoints: function generateEndpoints() {
    ...
    return new Promise(function generatorPromise(resolve, reject) {
      ...
      endpoints.forEach(function forEachEndpoint(endpointConfig) {
        ...
        //Template for list endpoints
        var listTemplate = `json.set(json.partial('pagination', {
  page: {
    lastEvaluatedKey: ${modelName}.lastEvaluatedKey,
    endpoint: endpoint,
    limit: ${modelName}.limit,
    count: ${modelName}.results.length
  }}));
...
`;
  ...
  createJoiSchema: function generateJoiSchema(modelName, schemaConfigs) {
    ...
    return new Promise(function getSchemaPromise(resolve, reject) {
      ...
      var joiSchemas = [];
      var mapping;
      var mappingFileAvailable = false;
      var promises = [];
      var filename = _.replace(_.startCase(modelName), ' ', '__').toLowerCase();
      ...
      schemaConfigs.forEach(function forEachSchema(schema) {
        ...
        var key = mapping[schemaName] || (mappingFileAvailable ? schemaName : _.camelCase(schemaName.replace(/__\w$/, '')));

        mapping[schemaName] = mapping[schemaName] || _.camelCase(schemaName.replace(/__\w$/, ''));
