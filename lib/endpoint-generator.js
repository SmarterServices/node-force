...
class EndpointGenerator {
  ...
  getRoute(routeType) {
    ...
    var method = endpointMethodMap[routeType],
      reply = `utils.replyJson('partials/${this.fileName}', {${this.displayName}: data}, hapi);`,
  ...
  options: {
    handler: function (request, hapi) {
      ...
      return ${this.displayName}Handler
        .${methodName}(opts)
        .then((data) => {
          return ${reply}
        })
        .catch((error) => {
          return errorResponse.formatError(error, null, hapi);
        });
    },
  ...
  ${methodName}(data) {
    ...
    return ${this.displayName}Service
      .${methodName}(data)
  }`;
  }
