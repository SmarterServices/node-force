...
var accountMapping = require('./../../config/routes/schema/account.json');
var contactMapping = require('./../../config/routes/schema/contact.json');
...
var herokuConnect = {
   ...
   * @param transaction {Object|null}
   * @returns {Promise}
   * @resolve {Object} The inserted data
   * @reject {Error} Sequelize error
   */
  addData: function (modelName, data, transaction) {
  ...
  /**
   * Create a user with account and contact
   * @param userData
   * @return {Promise}
   */
  createUser: function createUser(userData) {
    ...
    // if any of the promises that carries the transaction fails
    // the whole insertion will be rolled back
    return sequelize.transaction(function (transaction) {
      //Convert the object keys by using keys from the mapping
      var accountData = Utils.getMappedObject(userData.account, accountMapping, true);
      var data;

      //Add data for account
      return _this
        .addData('account', {payload: accountData}, transaction)
        .then(function addContact(accountData) {
          data = accountData;
          delete userData.account;
          userData.accountId = accountData.Id;

          //Convert the object keys by using keys from the mapping
          var contactData = Utils.getMappedObject(userData, contactMapping, true);
          return _this
            .addData('contact', {payload: contactData}, transaction)
            .then(function (contactData) {

              //Revert the object keys by using keys from the mapping
              var cont = Utils.getMappedObject(contactData, contactMapping, false);
